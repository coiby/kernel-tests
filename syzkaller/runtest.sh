...
rlJournalStart
    rlPhaseStartSetup
        ...
        export PATH="$PATH:/root/go${GO_VERSION}/go/bin"
    ...
    rlPhaseStartTest "fuzzing with syzkaller"
        # Run syzkaller
        start_time=$(date +%s)
        rlWatchdog "./bin/syz-manager ${verbose} -config syzkaller.conf" "${time}"
        end_time=$(date +%s)
        ...
        rlRun "./bin/syz-db unpack ${syzkaller_workdir}/corpus.db ${syzkaller_workdir}/corpus_dir"
