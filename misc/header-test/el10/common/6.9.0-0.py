...
def setup(exc):
    ...
    exc['internal/cpumap.h'] = ([], WARN | DENYLIST,
                                'error: #warning Dependendent on kernel-devel')

    exc['internal/evlist.h'] = ([], WARN | DENYLIST,
                                'error: #warning Dependendent on kernel-devel')

    exc['internal/evsel.h'] = ([], WARN | DENYLIST,
                               'error: #warning Dependendent on kernel-devel')

    exc['internal/mmap.h'] = ([], WARN | DENYLIST,
                              'error: #warning Dependendent on kernel-devel')

    exc['internal/rc_check.h'] = ([], WARN | DENYLIST,
                                 'error: #warning Dependendent on kernel-devel')

    exc['internal/threadmap.h'] = ([], WARN | DENYLIST,
                                  'error: #warning Dependendent on kernel-devel')

    exc['internal/xyarray.h'] = ([], WARN | DENYLIST,
                                 'error: #warning Dependendent on kernel-devel')

    exc['linux/android/binder.h'] = ([PID_T],
                                     OK, 'pid_t uid_t')

    exc['linux/android/binderfs.h'] = ([PID_T],
                                       OK, 'pid_t uid_t')

    exc['linux/coda.h'] = (['sys/types.h', '* #define _LINUX_TIME_H', 'linux/posix_types.h'],
                           OK | WARN, 'u_short pid_t ino_t caddr_t  u_short __kernel_pid_t, sys/types conflicts with linux/time.h included by coda.h')
