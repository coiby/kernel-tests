...
sed -i 's|rstrnt-reboot|if [ "${CKI_ABORT_RECIPE_ON_LWD:-0}" -eq "1" ]; then rstrnt-abort recipe; else rstrnt-abort --server $RSTRNT_RECIPE_URL/tasks/$RSTRNT_TASKID/status\nrstrnt-reboot;fi|' /usr/share/restraint/plugins/localwatchdog.d/99_reboot
