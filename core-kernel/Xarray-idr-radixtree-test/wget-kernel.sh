...
# get updated by liwang@redhat.com with cleanup and let it choose nearest lab.
...
Usage(){
    echo "Usage"
    echo "	$(basename $0) --nvr [version-number] --arch [arch] --rpm | --srpm | --debuginfo | --kabi | --print | --internal | --ktest | --ckirepo | --running"
    echo "Example"
...
init_vars_repo_cki()
{
    ...
    if test -n "$cki_repo"; then
        eval echo "Using cki repo $cki_repo for package download"
    ...
    if test -n "$def_url_cki"; then
        # 3.10.0-1160.21.1.el7.test.x86_64
        local kvra=$(basename $def_url_cki)
        # 3.10.0-1160.21.1.el7.test
        local cki_krpm
        local kname
        local ckirpm
        for kname in $kernel_names; do
            ckirpm="${kname}-${kvra}.rpm"
            echo "Checking existence of $def_url_cki/$ckirpm"
            check_existence "$def_url_cki/$ckirpm"
            [ $? = 0 ] && echo "Using cki kernel $def_url_cki/$ckirpm" && cki_krpm=$ckirpm && break
        done
    fi
    ((use_cki_kernel)) && test -z "$cki_krpm" && echo "Failed to fetch $def_url_cki/$ckirpm" && exit 1
}
...
workaround_cki_kdevel_binary()
{
    ((use_cki_kernel)) || return
    uname -r | grep -q x86_64 && return
    echo $list_url | grep -q dev_url || return
    echo "Workaroud for binary issue for cki kernel-devel build on non-x86 archs"
    echo "https://gitlab.com/cki-project/cki-tools/-/issues/28"
    rpm -q openssl-devel &>/dev/null || yum -y install openssl-devel &>/dev/null
    find /lib/modules/$(uname -r)/build  -type f -executable | xargs -I'{}' sh -c 'file $1 | grep x86 && rm $1 -f' -- {}
...
init_vars()
{
    ...
    uname -r | grep -q fc && ROOT_URL=https://kojipkgs.fedoraproject.org/packages
    ...
    for pkg_name in  $kernel_names; do
        ...
        echo "Checking existence of $(switch_to_final_url $doc_url)" 1>&2
        curl --head -s -f $(switch_to_final_url $doc_url) -o /dev/null && found=1 && break
        ((found == 1)) && break
    done
    ...
    if [ "$found" = 0 ]; then
        ...
        for pkg_name in  $kernel_names; do
            ...
            path_prefix=${def_url}/$sub_path/${version}/${release}
            doc_url="${path_prefix}/$arch/${sub_name}-devel-${version}-${release}.$arch.rpm"
            echo "Checking existence of $doc_url" 1>&2
            curl --head -s -f $(switch_to_final_url $doc_url) -o /dev/null && found=1 && break
        done
    fi

    ((found==0)) && echo "failed to find the $(switch_to_final_url $doc_url)" 1>&2 && exit
    echo "${def_url_cki:-$path_prefix} existed." 1>&2
...
function download_rpm()
{
    ...
    if ((install)); then
        ...
        [ $ret -ne 0 ] && echo "yum localinstall $pkg failed"
        workaround_cki_kdevel_binary
    fi
}

# ------- start ------------
TEMP=$(getopt -o vd:aipt -l cki:,brewrepo:,brew:,ckirepo:,srpm,rpm,kabi,perf,fw,install,arch:,debuginfo,debugkernel,internal,int,ktest,curr,running,nvr:,devel,print, -n 'example.bash' -- "$@")
...
while true ; do
    case "$1" in
        ...
        --nvr)
            echo "$2" | grep "^kernel" -qE && kernel_names=${2%%-[0-9]*}
