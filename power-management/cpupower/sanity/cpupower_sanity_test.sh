...
if echo "$cpupower_out" | grep 'no or unknown cpufreq driver is active on this CPU' >/dev/null; then
        echo "UNSUPPORTED FEATURE"
        echo "This test will fail, because there is'nt enough return values in Beaker"
        echo "It is written to work with jenkins_results script, which can distinguish these two states"
        echo "by the string UNSUPPORTED FFEATURE standing alone on one line."
        exit 1
fi
