...
parser.add_option("-d", "--dev", dest="test_dev", default=None,
                  help="Specify the device to use, must be set on the command line.", metavar="FILE",
                  type="string", action="store")
parser.add_option("-t", "--tld", dest="tld", default="/mnt/testarea/USB-storage",
                  help="Specify the top level directory device to use, default is /mnt/testarea/USB-storage",
...
mount_pt = tld + "/USB_mnt"
scratch_dir = tld + "/scratch"
log_dir = tld + "/logs"

logger = logging.getLogger()
handler = logging.StreamHandler()
logfh = logging.FileHandler(log_dir + "/test_usb.log")
...
if not options.test_dev:
    logger.critical("Test device must be set on the command line")
    sys.exit(1)
...
def run_test():
    ...
    for t in iotype:
        ...
        if dt_results[t][2] != 0:
            logger.warning("dt " + t + " run exited with nonzero return code!! RC=" + str(dt_results[t][2]))
        cw.drop_caches()
...
def main(test_dev):
    ...
    if test_dev in bd.usb_scsi_devs:
        # run our test
       ...
       #     mydt.test_size = bd.get_size_in_bytes(test_dev) - 2048
        ...
        print "{0} does not appear to be a USB device".format(test_dev)
        return 1
