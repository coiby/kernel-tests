...
if test -z "${DMA_MODNAME}"; then
	test_complete "SKIP" "${RSTRNT_TASKNAME} The parameter DMA_MODNAME must be set"
fi

if test -z "${DMA_IOMMU_CONF}"; then
	test_complete "SKIP" "${RSTRNT_TASKNAME} The parameter DMA_IOMMU_CONF must be set"
fi
...
rlJournalStart
{
	...
	case "${DMA_MODNAME}" in
		...
		"ptdma" | "ioatdma")
			;;
		*)
			test_complete "SKIP" "${DMA_TESTNAME}: DMA_MODNAME ${DMA_MODNAME} is not recognized"
			;;
	esac
	...
	case "${DMA_MODNAME}" in
		"idxd")
			rlPhaseStartTest "${DMA_TESTNAME} memcpy-1-thread"
			{
				dmaengine_enable "dsa0" "wq0.0" "engine0.0"
				dmaengine_test "dsa0" "wq0.0" 3000 1000 0 1 "${DMA_TESTNAME} $(get_config) dmatest memcpy-1-thread"
			...
			if ! test -z "${DMATEST_MULTITHREADS}"; then
				...
				rlPhaseStartTest "${DMA_TESTNAME} memcpy-${DMATEST_MULTITHREADS}-thread"
				{
					dmaengine_enable "dsa0" "wq0.0" "engine0.0"
		...
		"ioatdma" | "ptdma")
			...
			rlPhaseStartTest "${DMA_TESTNAME} memcpy testing"
			{
				dmatest_run 3000 1000 0 1 "${DMA_TESTNAME} memcpy-1-thread"
				if ! test -z "${DMATEST_MULTITHREADS}"; then
					echo "DMA_MULTITHREADS = ${DMATEST_MULTITHREADS}"
	...
	rstrnt-report-result "${DMA_TESTNAME} dmatest test complete" "PASS"
}
