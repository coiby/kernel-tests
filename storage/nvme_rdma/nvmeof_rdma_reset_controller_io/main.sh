...
function client {
	...
	rstrnt-sync-set -s "CLIENT_FIO_RESET_CONTROLLER_DONE"
...
function server {
	...
	tlog "--- wait client to set CLIENT_FIO_RESET_CONTROLLER_DONE ---"
	rstrnt-sync-block -s "CLIENT_FIO_RESET_CONTROLLER_DONE" ${CLIENTS}
