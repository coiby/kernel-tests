...
function client {
	...
	do
		tok "echo 1 > ${sysfs}/rescan_controller"
		ret=$?
		if [ $ret -eq 1 ]; then
			echo "rescan_controller operation failed: $num"
			break
		fi
		sleep 0.1
	...
	[ $ret -eq 0 ] && tlog "INFO: rescan/reset controller operation pass:$num"

	# wait background fio operation done
	tlog "INFO: wait fio operation done"
	wait

	rhts_sync_set -s "CLIENT_FIO_RESCAN_RESET_CONTROLLER_IO_DONE"
...
function server {
	...
	rhts_sync_block -s "CLIENT_FIO_RESCAN_RESET_CONTROLLER_IO_DONE" ${CLIENTS}
