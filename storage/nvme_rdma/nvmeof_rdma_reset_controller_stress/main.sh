...
function client {
	tlog "--- wait server to set 14_SERVER_NVMEOF_RDMA_TARGET_SETUP_READY ---"
	rstrnt-sync-block -s "14_SERVER_NVMEOF_RDMA_TARGET_SETUP_READY" "${SERVERS}"
	...
	rstrnt-sync-set -s "14_CLIENT_RESET_CONTROLLER_DONE"

	#disconnect the target
	NVMEOF_RDMA_DISCONNECT_TARGET n testnqn

	rstrnt-sync-set -s "14_CLIENT_DISCONECT_TARGET_DONE"
}
...
function server {
	...
	if [ $ret -eq 0 ]; then
		...
		rstrnt-sync-set -s "14_SERVER_NVMEOF_RDMA_TARGET_SETUP_READY"
	else
		tlog "INFO: NVMEOF_RDMA_Target_Setup failed, test_protocol:$test_protocol"
		return 1
	fi

	tlog "--- wait client to set 14_CLIENT_RESET_CONTROLLER_DONE ---"
	rstrnt-sync-block -s "14_CLIENT_RESET_CONTROLLER_DONE" "${CLIENTS}"

	tlog "--- wait client to set 14_CLIENT_DISCONECT_TARGET_DONE ---"
	rstrnt-sync-block -s "14_CLIENT_DISCONECT_TARGET_DONE" "${CLIENTS}"
