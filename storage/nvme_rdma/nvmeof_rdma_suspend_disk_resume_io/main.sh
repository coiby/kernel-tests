...
function client {
	tlog "--- wait server to set 19_SERVER_NVMEOF_RDMA_TARGET_SETUP_READY ---"
	rstrnt-sync-block -s "19_SERVER_NVMEOF_RDMA_TARGET_SETUP_READY" "${SERVERS}"
	...
	if [ $ret -ne 0 ]; then
		...
		rstrnt-sync-set -s "19_CLIENT_DISCONECT_TARGET_DONE"
		return 1
	else
	...
	rstrnt-sync-set -s "19_CLIENT_CONNECT_TARGET_DONE"
}
...
function server {
	...
	if [ $ret -eq 0 ]; then
		...
		rstrnt-sync-set -s "19_SERVER_NVMEOF_RDMA_TARGET_SETUP_READY"
	else
		tlog "INFO: NVMEOF_RDMA_Target_Setup failed, test_protocol:$test_protocol"
		return 1
	fi

	tlog "--- wait client to set 19_CLIENT_CONNECT_TARGET_DONE---"
	rstrnt-sync-block -s "19_CLIENT_CONNECT_TARGET_DONE" "${CLIENTS}"
