...
if [ ! "$ISCSI_BIG" ]; then
  setup="--setup iscsi --setup vdo/setup/storage/cleanup_multipath --filter tags:iscsi_10G"
else
  setup="--setup iscsi_big --setup vdo/setup/storage/cleanup_multipath --filter tags:iscsi_2T"
fi

command="stqe-test run --fmf $component_filter $setup $ENV_PARAMS"
echo "$command"
