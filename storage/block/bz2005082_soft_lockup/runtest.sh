...
function run_test()
{
    ...
    rlRun "modprobe scsi_debug dev_size_mb=2000 dif=1 dix=1 delay=0 add_host=1"
    sleep 5
    dev=`ls -d /sys/bus/pseudo/drivers/scsi_debug/adapter*/host*/target*/*/block/* | head -1 | xargs basename`

    rlRun "echo 2 > /sys/block/$dev/queue/rq_affinity"
    sleep 3

    rlRun "fio --name=r_4k --filename=/dev/$dev --ioengine=sync \
