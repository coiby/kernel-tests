...
class Test(functional.connection.functional_connection_base.FunctionalConnectionBaseTest):
    ...
    def determine_connection_information(self):
        ...
        if connection_preset != 'custom':
            ...
            # Use the preset connection specified
                ...
                for key, value in settings.items():
                    bluetooth_connection_settings[key] = value
                    print("{0} -> {1}".format(bluetooth_connection_settings[key], value))
        ...
        for key, value in bluetooth_connection_settings.items():
    ...
    def find_remote_bluetooth_device(self):
        ...
        adapter = manager.get_default_local_device()
        if not adapter:
    ...
    def obex_file_transfer_setup(self):
        ...
        try:
            ...
            raise Exception("Failed to create temp directory: {0}".format(str(e)))
        ...
        try:
            ...
            for f in files:
                ...
                if not name == "test-move.txt":
                    raise Exception("Unknown file in tempdir: {0}".format(name))
    ...
    def obex_object_push_setup(self):
        ...
        try:
            ...
            raise Exception("Failed to create temp file: {0}".format(str(e)))
    ...
    def obex_object_push(self):
        ...
        #pull, send, get, verify
        #pull can only get a 'vcard.vcf', whereas 'get' can get anything
        try:
            obex.opp_pull_business_card(tempf)
            obex.opp_sendfile(tempf)
            obex.ftp_getfile(tempf + '.cp', rtempf)
    ...
    def obex_pbap(self):
        ...
        try:
            ...
            if size != 2:
                raise Exception("PBAP mismatch size: expected {0}, got {1}".format('2', size))
