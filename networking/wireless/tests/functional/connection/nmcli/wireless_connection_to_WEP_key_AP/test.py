...
"""
The functional.connection.nmcli.wireless_connection_to_WEP_key_AP
...
class Test(functional.connection.nmcli.connection_base.ConnectionBaseTest):
    ...
    def __init__(self):
        ...
        self.set_test_name(name = '/kernel/wireless_tests/functional/connection/nmcli/wireless_connection_to_WEP_key_AP')
        self.set_test_author(name = 'Ken Benoit', email = 'kbenoit@redhat.com')
        self.set_test_description(description = 'Establish a wireless connection to a WEP key AP using nmcli.')

        self.replace_test_step_description(
            pattern = 'Configure the test wireless interface',
            new_description = 'Configure the test wireless interface with a WEP key-protected connection using nmcli',
        )
