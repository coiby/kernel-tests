...
"""
The worknode.linux.manager.rhel8.network module provides a class
...
import worknode.linux.manager.rhel8.network_interface.vendor_manager_factory
import worknode.linux.util.rhel8.nmcli
...
class NetworkManager(worknode.linux.manager.network_base.NetworkManager):
    ...
    def __configure_executables(self):
        ...
        nmcli = self.add_command(
            command_name = 'nmcli',
            command_object = worknode.linux.util.rhel8.nmcli.nmcli(
        ...
        # Changing from the Unicode escape regular expression since we there is
        # a need with python3 to change the output from a byte string and decode
        # that into a UTF-8 string. The decode is done at the work node level so
        # that the output coming out of a process object has been already
        # decoded.
        wget.set_local_file_regular_expression(
            regex = '\d+-\d+-\d+ \d+:\d+:\d+ \(\d+\.?\d*? \wB\/s\) \S \S(?P<file_name>.+?)\S saved',
        )
...
class WirelessInterface(worknode.linux.manager.network_base.WirelessInterface):
    """
    WirelessInterface represents a wireless network interface on a RHEL8 work
    node.
    ...
    def __init__(self, parent, name):
        ...
        self._set_vendor_manager_factory(
            factory = worknode.linux.manager.rhel8.network_interface.vendor_manager_factory.VendorManagerFactory(),
        )
...
class WiredInterface(worknode.linux.manager.network_base.WiredInterface):
    """
    WiredInterface represents a wired network interface on a RHEL8 work node.
