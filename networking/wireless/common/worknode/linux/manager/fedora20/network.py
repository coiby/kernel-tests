...
"""
The worknode.linux.manager.fedora20.network module provides a class
...
import worknode.linux.util.fedora20.nmcli
...
class NetworkManager(worknode.linux.manager.network_base.NetworkManager):
    ...
    def __configure_property_manager(self):
        ...
        dev_object = nmcli.get_nmcli_object(object_string = 'device')
    ...
    def __configure_executables(self):
        ...
        nmcli = self.add_command(
            command_name = 'nmcli',
            command_object = worknode.linux.util.fedora20.nmcli.nmcli(
        ...
        # escape out the quotes from the rest of the file name
        wget.set_local_file_regular_expression(
            regex = '- \W(?P<file_name>.+?)\W saved',
        )
...
class WirelessInterface(worknode.linux.manager.network_base.WirelessInterface):
    """
    WirelessInterface represents a wireless network interface on a Fedora 20
    work node.
    ...
    def __init__(self, parent, name):
        ...
        dev_object = nmcli.get_nmcli_object(object_string = 'device')
        show_command = dev_object.get_command(command_string = 'show')
        nmcli_command = show_command.get_command(
            command_arguments = '{0}'.format(self.get_name()),
        )
        nmcli_parser = show_command.get_command_parser()
    ...
    def enable(self, preferred_command = 'nmcli'):
        ...
        if preferred_command == 'nmcli':
            ...
            nmcli_con = nmcli.get_nmcli_object(object_string = 'connection')
            nmcli_con_up = nmcli_con.get_command(command_string = 'up')
            nmcli_con_up.run_command(
                command_arguments= 'id {0}'.format(self.get_name()),
            )
    ...
    def disable(self, preferred_command = 'nmcli'):
        ...
        if preferred_command == 'nmcli':
            ...
            nmcli_con = nmcli.get_nmcli_object(object_string = 'connection')
            nmcli_con_down = nmcli_con.get_command(command_string = 'down')
    ...
    def configure_wireless_connection(
        ...
        if preferred_command == 'nmcli':
            ...
            nmcli_gen = nmcli.get_nmcli_object(object_string = 'general')
        ...
        # If the preferred command is wpa_supplicant
            ...
            if hidden:
                wpa_supplicant_conf.set_network_scan_ssid(
                    property_value = '1',
                )
            ...
            if enable_debug_logging:
                ...
                # TODO: Abstract out dbus-send command
                self._get_work_node().run_command(
                    command = 'dbus-send --system --print-reply '
    ...
    def delete_wireless_connection(
        ...
        if preferred_command == 'nmcli':
            ...
            nmcli_con = nmcli.get_nmcli_object(object_string = 'connection')
            nmcli_con_delete = nmcli_con.get_command(command_string = 'delete')
            nmcli_con_down = nmcli_con.get_command(command_string = 'down')
            command_arguments = 'id "{0}"'.format(network_name)
            nmcli_con_down.run_command(
                command_arguments = command_arguments,
                timeout = 60,
            )
            nmcli_con_delete.run_command(
                command_arguments = command_arguments,
                timeout = 60,
            )
    ...
    def disable_radio(self, preferred_command = 'nmcli'):
        ...
        if preferred_command == 'nmcli':
            ...
            nmcli_radio = nmcli.get_nmcli_object(object_string = 'radio')
            nmcli_radio_wifi = nmcli_radio.get_command(command_string = 'wifi')
            nmcli_radio_wifi.run_command(command_arguments = 'off')
            radio_status = nmcli_radio_wifi.run_command()
            if radio_status['radio_status'] == 'enabled':
                ...
                raise FailedCommandOutputError(
                    "Unknown if the wireless radio is disabled"
                )
    ...
    def enable_radio(self, preferred_command = 'nmcli'):
        ...
        if preferred_command == 'nmcli':
            ...
            nmcli_radio = nmcli.get_nmcli_object(object_string = 'radio')
            nmcli_radio_wifi = nmcli_radio.get_command(command_string = 'wifi')
            nmcli_radio_wifi.run_command(command_arguments = 'on')
            radio_status = nmcli_radio_wifi.run_command()
            if radio_status['radio_status'] == 'disabled':
                raise FailedCommandOutputError(
                    "Failed to enable the wireless radio"
                )
            elif radio_status['radio_status'] != 'enabled':
                raise FailedCommandOutputError(
    ...
    def get_signal_strength(self, ssid, preferred_command = 'nmcli'):
        ...
        if preferred_command == 'nmcli':
            ...
            nmcli_dev = nmcli.get_nmcli_object(object_string = 'device')
            nmcli_dev_show = nmcli_dev.get_command(command_string = 'show')
            parsed_output = nmcli_dev_show.run_command(
...
class WiredInterface(worknode.linux.manager.network_base.WiredInterface):
    """
    WiredInterface represents a wired network interface on a Fedora 20 work
    node.
