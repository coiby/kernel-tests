...
"""
The worknode.linux.manager.rhel7.bluetooth module provides a class
...
import worknode.linux.util.rhel7.bluetoothctl
import worknode.linux.util.rhel7.bluez
import worknode.linux.util.rhel7.obex
...
class BluetoothManager(worknode.linux.manager.bluetooth_base.BluetoothManager):
    ...
    def __init__(self, parent):
        ...
        self.__commands = {
            'bluetoothctl': worknode.linux.util.rhel7.bluetoothctl.bluetoothctl(
                work_node = self._get_work_node(),
            ),
            'bluez'       : worknode.linux.util.rhel7.bluez.bluez(
                work_node = self._get_work_node(),
            ),
            'obex'       : worknode.linux.util.rhel7.obex.obex(
                work_node = self._get_work_node(),
            ),
        }
...
class LocalBluetoothDevice(worknode.linux.manager.bluetooth_base.LocalBluetoothDevice):
    ...
    def __configure_executables(self):
        ...
        bluetoothctl = self._add_command(
            ...
            command_object = \
                worknode.linux.util.rhel7.bluetoothctl.bluetoothctl(
                    work_node = self._get_work_node(),
                ),
        )
        bluez = self._add_command(
            command_name = 'bluez',
            command_object = \
                worknode.linux.util.rhel7.bluez.bluez(
                    work_node = self._get_work_node(),
        ...
        obex = self._add_command(
            command_name = 'obex',
            command_object = \
                worknode.linux.util.rhel7.obex.obex(
                    work_node = self._get_work_node(),
    ...
    def __configure_property_manager(self):
        ...
        property_manager.set_command_priority(
            ...
            command_priority = ['sysfs_mac_address', 'hcitool_dev'],
        )
...
class RemoteBluetoothDevice(worknode.linux.manager.bluetooth_base.RemoteBluetoothDevice):
    """
    RHEL7 child class for remote Bluetooth devices external from the work node.
    ...
    def __configure_executables(self):
        ...
        bluetoothctl = self._add_command(
            command_name = 'bluetoothctl',
            command_object = worknode.linux.util.rhel7.bluetoothctl.bluetoothctl(
                work_node = self._get_work_node(),
            ),
        )
        bluez = self._add_command(
