...
"""
The worknode.linux.manager.rhel7.file_system module provides a class
...
class FileSystemManager(worknode.linux.manager.file_system_base.FileSystemManager):
    ...
    def get_directory(self, directory_path):
        ...
        return worknode.linux.manager.rhel7.file_system.DirectoryObject(
            parent = self,
            path = directory_path,
        )
    ...
    def get_file(self, file_path):
        ...
        return worknode.linux.manager.rhel7.file_system.FileObject(
            parent = self,
            path = file_path,
        )
...
class FileObject(worknode.linux.manager.file_system_base.FileObject):
    ...
    def read_file(self):
        ...
        """
        work_node = self._get_work_node()
        output = work_node.run_command(command = 'cat {0}'.format(self._get_path()))
        return output
