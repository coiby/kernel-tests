...
"""
The worknode.linux.manager.rhel6.audio module provides a class (AudioManager)
...
class SoundcardControlFactory(worknode.linux.manager.audio_base.SoundcardControlFactory):
    ...
    def get_soundcard_control(self, parent, num_id):
        ...
        """
        import worknode.linux.manager.rhel6.soundcard.control
        control = None
        ...
        if control_type == 'INTEGER' and interface == 'MIXER':
            control = worknode.linux.manager.rhel6.soundcard.control.MixerIntegerSoundcardControl(
                parent = parent,
                num_id = num_id,
            )
        elif control_type == 'INTEGER' and interface == 'PCM':
            control = worknode.linux.manager.rhel6.soundcard.control.PCMIntegerSoundcardControl(
                parent = parent,
                num_id = num_id,
            )
        elif control_type == 'BOOLEAN':
            control = worknode.linux.manager.rhel6.soundcard.control.BooleanSoundcardControl(
                parent = parent,
                num_id = num_id,
            )
        elif control_type == 'ENUMERATED':
            control = worknode.linux.manager.rhel6.soundcard.control.EnumeratedSoundcardControl(
                parent = parent,
                num_id = num_id,
            )
        elif control_type == 'BYTES':
            control = worknode.linux.manager.rhel6.soundcard.control.BytesSoundcardControl(
                parent = parent,
                num_id = num_id,
            )
        elif control_type == 'IEC958':
            control = worknode.linux.manager.rhel6.soundcard.control.IEC958SoundcardControl(
                parent = parent,
                num_id = num_id,
            )
        else:
