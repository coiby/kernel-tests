...
class obex(worknode.linux.util.rhel8.bluez.bluez):
    """
    ...
    def setup_file_transfer(self):
        ...
        if not ftp:
            raise Exception("Could not setup remote file transfer")
    ...
    def ftp_putfile(self, source, target):
        if not os.path.isfile( source ):
            raise Exception("{0} does not exist".format(source))
    ...
    def map_listfolders(self, lfilter={}):
        ...
        for k in lfilter:
            if k not in filter_list:
                raise Exception ('Bad keyword in filter: {0}'.format(k))
    ...
    def map_push_message(self, source, folder, args=dict()):
        ...
        for a in args:
            if a not in args_list:
                raise Exception ('Bad arg: {0}'.format(a))
