...
"""
The worknode.linux.util.ip module provides a class (ip) that represents the ip
...
class ip(worknode.worknode_executable.WorkNodeExecutable):
    """
    ip represents the ip command line executable, which provides a command
    line interface that allows for showing and manipulating routing, devices,
    policy routing, and tunnels.

    """
    def __init__(self, work_node, command = 'ip'):
        super(ip, self).__init__(work_node)
        self.__objects = {}
    ...
    def add_ip_object(self, object_string):
        """
        Adds an ip queriable object.
        ...
        self.__objects[object_string] = worknode.linux.util.ip.ip.IpObject(
    ...
    def get_ip_object(self, object_string):
        """
        Get an ip queriable object.
        ...
        if object_string not in self.__objects:
            raise NameError("ip object is not defined for {0}".format(object_string))
        return self.__objects[object_string]

    class IpObject(worknode.worknode_executable.WorkNodeExecutable):
        """
        ip has various objects it can query. The objects have various commands
        that can be executed with them.

        """
        def __init__(self, object_string, work_node, parent_object):
            super(worknode.linux.util.ip.ip.IpObject, self).__init__(work_node = work_node)
        ...
        def add_command(self, command_string):
            ...
            Keyword arguments:
            command_string - The string that needs to be supplied to the ip
            ...
            self.__commands[command_string] = worknode.linux.util.ip.ip.IpObject.IpObjectCommand(
        ...
        def get_command(self, command_string):
            """
            Get the command for the ip object.
            ...
            if command_string not in self.__commands:
                raise NameError("ip object command is not defined for {0}".format(command_string))
            return self.__commands[command_string]

        class IpObjectCommand(worknode.worknode_executable.WorkNodeExecutable):
            """
            ip objects have various commands it can execute.

            """
            def __init__(self, command_string, work_node, parent_object):
                super(worknode.linux.util.ip.ip.IpObject.IpObjectCommand, self).__init__(work_node = work_node)
            ...
            def initialize_command_parser(self, output_type):
                ...
                if output_type == 'key-value':
                    ...
                    self.__command_parser = worknode.command_parser.TableRowParser()
                else:
                    raise Exception("Unknown output type provided")
            ...
            def run_command(self, command_arguments = None, timeout = 5):
                ...
                output = super(worknode.linux.util.ip.ip.IpObject.IpObjectCommand, self)._run_command(command = full_command, timeout = timeout)
            ...
            def get_command(self, command_arguments = None):
                ...
                ip_string = self._get_parent_object()._get_parent_object().get_command()

                full_command += ip_string + ' '
