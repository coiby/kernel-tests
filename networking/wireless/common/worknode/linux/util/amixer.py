...
"""
The worknode.linux.util.amixer module provides a class (amixer) that represents
the amixer command line executable.
...
class amixer(worknode.worknode_executable.WorkNodeExecutable):
    """
    amixer represents the amixer command line executable, which provides a
    command line mixer for ALSA soundcard drivers.

    """
    def __init__(self, work_node, command = 'amixer'):
        super(amixer, self).__init__(work_node = work_node)
        self.__commands = {}
    ...
    def add_amixer_command(self, command_string):
        """
        Adds an amixer command.

        Keyword arguments:
        command_string - The long form string that needs to be supplied to
                         amixer to execute the command.
        ...
        self.__commands[command_string] = worknode.linux.util.amixer.amixer.AmixerCommand(
    ...
    def get_amixer_command(self, command_string):
        """
        Get an amixer command.
        ...
        if command_string not in self.__commands:
            raise NameError("amixer command is not defined for {0}".format(command_string))
        return self.__commands[command_string]

    class AmixerCommand(worknode.worknode_executable.WorkNodeExecutable):
        """
        amixer has various command it can execute.

        """
        def __init__(self, command_string, work_node, parent_object):
            super(worknode.linux.util.amixer.amixer.AmixerCommand, self).__init__(work_node = work_node)
        ...
        def run_command(self, command_arguments = None, timeout = 5):
            ...
            output = super(worknode.linux.util.amixer.amixer.AmixerCommand, self)._run_command(command = full_command, timeout = timeout)
        ...
        def get_command(self, command_arguments = None):
            ...
            amixer_string = self._get_parent_object().get_command()

            full_command += amixer_string + ' '
