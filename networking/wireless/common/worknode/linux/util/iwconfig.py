...
"""
The worknode.linux.util.iwconfig module provides a class (iwconfig) that 
represents the iwconfig command line executable.
...
class iwconfig(worknode.worknode_executable.WorkNodeExecutable):
    """
    iwconfig represents the iwconfig command line executable, which provides a 
    command line utility for gathering information about the wireless
    connection.

    """
    def __init__(self, work_node, command = 'iwconfig'):
        super(iwconfig, self).__init__(work_node)
        self.__objects = {}
    ...
    def run_command(self, command_arguments = None, timeout = 5):
        ...
        output = super(iwconfig, self)._run_command(command = full_command, timeout = timeout)
