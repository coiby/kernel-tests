...
static inline void unlock(unsigned long *flag)
{
	...
	__asm__ __volatile__("movl $0,%0; rep; nop" : "=m"(*flag) :: "memory");
#endif
}
...
static inline void test_TSC(unsigned long *shared)
{
	...
	if (delta < 0) {
		...
		if (delta < SHARED_LL(WORST_TSC)) {
			...
			fprintf(f, "\rnew TSC-warp maximum: %9Ld cycles, %016Lx -> %016Lx\n",
				delta, t0, t1);
		}
		unlock(&SHARED(LOCK));
	}
...
static inline void test_TOD(unsigned long *shared)
{
	...
	if (delta < 0) {
		...
		if (delta < SHARED_LL(WORST_TOD)) {
			...
			fprintf(f, "\rnew TOD-warp maximum: %9Ld usecs,  %016Lx -> %016Lx\n",
				delta, T0, T1);
		}
		unlock(&SHARED(LOCK));
	}
...
static inline void test_CLOCK(unsigned long *shared)
{
	...
	if (delta < 0) {
		...
		if (delta < SHARED_LL(WORST_CLOCK)) {
			...
			fprintf(f, "\rnew CLOCK-warp maximum: %9Ld nsecs,  %016Lx -> %016Lx\n",
				delta, T0, T1);
		}
		unlock(&SHARED(LOCK));
	}
