...
int max_pct = 100;
int seconds = 0;
int break_systime_centiseconds = 3000;
int rest_seconds = 0;
int work_seconds = 0;
...
void print_usage_and_exit(char *prog_name) {
    ...
    fprintf(stderr, "-l <LOAD>, one of { 'spin', 'sleep', 'int', 'fpu', 'mem', 'dirty', 'splice' }\n");
...
#define DEFAULT_SPLICE_BYTES (64*1024)
void splice_load(int tix)
{
        int fd_in, fd_out, fds[2];
        size_t splice_bytes = DEFAULT_SPLICE_BYTES;

        if (pipe(fds) < 0) {
                printf("thread-%d: pipe failed, errno=%d (%s)\n",
                                thread[tix].tid, errno, strerror(errno));
                return;
        }   


        if ((fd_in = open("/dev/zero", O_RDONLY)) < 0) {
                printf("thread-%d: open /dev/zero failed, errno=%d (%s)\n",
                                thread[tix].tid, errno, strerror(errno));
                (void)close(fds[0]);
                (void)close(fds[1]);
        }   
        if ((fd_out = open("/dev/null", O_WRONLY)) < 0) {
                printf("thread-%d: open /dev/null failed, errno=%d (%s)\n",
                                thread[tix].tid, errno, strerror(errno));
                (void)close(fd_in);
                (void)close(fds[0]);
                (void)close(fds[1]);
        ...
        for (;;) {
                ...
                if (ret < 0) {
                        printf("thread-%d: splice() write pipe failed, errno=%d (%s)\n",
                                thread[tix].tid, errno, strerror(errno));
                        break;
                }
                ...
                ret = splice(fds[0], NULL, fd_out, NULL,
                                splice_bytes, SPLICE_F_MOVE);
                if (ret < 0) {
                        printf("thread-%d: splice() read pipe failed, errno=%d (%s)\n",
                                thread[tix].tid, errno, strerror(errno));
                        break;
                }
...
void *run(void *arg) {
    ...
    if (do_splice_load)   {   splice_load(tix); }
...
void stop_threads(int proc_ix) {
    for (int ix = 1;  (ix < threads);  ix++) {
        thread[ix].flag_quit = 1;
    }
}

int check_proc_stime(void)
{
	static struct statStuff stat = {};
	readStat(getpid(), &stat);
	/* 30s, ideally 30000s, here for reduce runtime*/
	if (stat.stime > break_systime_centiseconds) {
		printf("stime=%ld ,breaking\n", stat.stime);
		return 1;
	}
	return 0;
}

void *run_checker(void *args)
{
	for (;;) {
		if (check_proc_stime()) {
			printf("Stopper %d stopped", getpid());	
			stop_threads(0);
		}	
	}
}
...
void make_threads(int proc_ix) {
    ...
    thread = malloc((threads + 1)* sizeof(thread_data_t));
    if (thread == NULL) {
...
int main(int argc, char *argv[]) {
    int opt;
    while ((opt = getopt(argc, argv, "b:dGhHi:Ik:l:L:m:MNn:p:qrs:St:TvVw")) != -1) {
	switch (opt) {
	    case 'b': break_systime_centiseconds = atoi(optarg); break;
	    ...
	    case 'l':
                ...
                do_splice_load   = !strcmp(optarg, "splice");
            break;
