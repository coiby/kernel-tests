...
rlJournalStart
    if [ "${REBOOTCOUNT}" -eq 0 ]; then
        ...
        rlPhaseStartTest
            rlRun "pushd ../src"
            rlRun "make all"
            # Un-mapped crash test
            rlRun "nohup echo 1 > /sys/kernel/vkm/write_um_crash" 139 "(SEGFAULT expected)"
            sleep 1
            rlRun "dmesg > dmesg-crash.log"
            rlAssertGrep "Unable to handle kernel paging request" dmesg-crash.log
